...
- Testing approaches
- Plan for updating the Change Documentation file
...
- Reading the "Change Documentation" before making code changes
- Updating the "Change Documentation" after each code change
...
1. Read the "Change Documentation" to understand previous modifications.
2. Strictly implement changes according to the plan (checklist items).
...
4. After completing the implementation of a checklist item, **use file tools** to append to "Task Progress" (as a standard step of plan execution). First, use the terminal command to get the current timestamp:
   ```
   ...
   date "+%Y-%m-%d %H:%M:%S"
   ```
   Then, add the entry to Task Progress:
   ```
   [Current system timestamp from command above]
   ...
   ```
5. Update the "Change Documentation" with details of the current change:
   ```
   ## Change #[N]
   ...
   *   Status: Pending
   ```
6. Request user confirmation and feedback: `Please review the changes for step [X] and test the functionality. Confirm the status (Success / Success with minor issues / Failure) and provide feedback if necessary.`
7. Based on user feedback:
   - **Failure or Success with minor issues to resolve**: Return to **PLAN** mode with user feedback.
   - **Success**: Update the Change Documentation to mark the change as "Fixed". If the checklist has unfinished items, proceed to the next item; if all items are complete, enter **REVIEW** mode.
...
- Review the "Change Documentation" for completeness and accuracy
...
- Before each code change, read the "Change Documentation" to understand previous modifications
- After each code change, update the "Change Documentation" with the new modification details
- After each change, prompt the user to test the functionality and provide feedback
- Always use system commands to get accurate timestamps when recording date and time information
...
Created On: [System timestamp - Use terminal command to get current date and time]
...
*   [System timestamp - Use terminal command to get current date and time]
    *   Step: [Checklist item number and description]
...
# Change Documentation (Populated by EXECUTE mode after each code change)
- Requirement: [User's original requirement]
- Reason: [Reason for the change]
- Purpose: [Purpose of the change]
- File: [File path]
