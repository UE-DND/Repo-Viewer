...
const Row = React.memo(({ data, index, style }: ListChildComponentProps) => {
  ...
  const adjustedStyle = {
    ...style,
    paddingBottom: FILE_ITEM_CONFIG.spacing.visualMarginBottom, // 使用更小的视觉间距
...
const FileList = React.memo<FileListProps>(({
  ...
  const rowHeight = useMemo(() => {
    ...
    // 上下内边距
    const paddingY = isSmallScreen 
      ? FILE_ITEM_CONFIG.spacing.paddingY.xs * 2 
      : FILE_ITEM_CONFIG.spacing.paddingY.sm * 2;
    
    // 下边距
    const marginBottom = FILE_ITEM_CONFIG.spacing.marginBottom;
    
    // 计算总高度
    return baseHeight + paddingY + marginBottom;
  }, [isSmallScreen]);
  ...
  useEffect(() => {
    ...
    const calculateAvailableHeight = () => {
      ...
      // 计算理论上的最大可用高度
      const maxAvailableHeight = viewportHeight - topElementsHeight - bottomMargin;
      ...
      const needsScrolling = contentHeight > maxAvailableHeight || (!isSmallScreen && fileCount >= 10);
      ...
      if (!needsScrolling) {
        ...
        const nonScrollHeight = contentHeight + 
        ...
        const minHeight = Math.max(
          rowHeight + LIST_HEIGHT_CONFIG.nonScrollPadding.top + LIST_HEIGHT_CONFIG.nonScrollPadding.bottom + hoverExtraSpace * 2,
  ...
  }, [contents.length, rowHeight, isSmallScreen, hasReadmePreview, hoverExtraSpace]); // 添加 hoverExtraSpace 作为依赖项
  ...
  const listPadding = useMemo(() => {
    ...
    if (!needsScrolling) {
      // 考虑悬停效果所需的额外空间
      const verticalPadding = Math.floor((LIST_HEIGHT_CONFIG.nonScrollPadding.top + hoverExtraSpace) / 2);
      return {
  ...
  return (
    <Box 
      sx={{ 
        ...
        borderColor: 'divider',
        p: containerPadding,
        pr: needsScrolling ? { xs: containerPadding.xs + 0.5, sm: containerPadding.sm + 0.5 } : containerPadding,
