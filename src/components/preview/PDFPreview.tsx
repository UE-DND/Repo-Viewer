...
import { Box, Typography, useTheme, IconButton, Tooltip, CircularProgress, Dialog, DialogTitle, DialogContent, DialogContentText, DialogActions, Button } from '@mui/material';
import { Worker, Viewer, SpecialZoomLevel, RenderError } from '@react-pdf-viewer/core';
...
interface PDFPreviewProps {
  ...
  lastVisiblePagesRef: React.MutableRefObject<{page: number, ratio: number}[]>;
    ...
    if (showPdf && !loading && isSmallScreen && viewerRef.current) {
      ...
      const handleTouchMove = (e: TouchEvent) => {
        if (isPinching && e.touches.length === 2) {
          ...
          // 仅在放大状态下支持拖动
          const pdfViewer = container.querySelector('.rpv-core__viewer-container');
          if (pdfViewer) {
    ...
    >
      ...
      >
        ...
        <DialogActions>
          ...
          <Button onClick={handleSecurityCancel} color="primary" variant="contained" autoFocus>
            取消
          </Button>
        </DialogActions>
      </Dialog>
